      ...
      <header className="sticky top-0 z-30 border-b border-[var(--color-border)] bg-[var(--color-surface)]/95 backdrop-blur">
        <div className="mx-auto flex min-h-20 max-w-[1600px] items-center justify-between gap-4 px-4 md:px-8">
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <button onClick={() => void logout()} className="min-h-11 rounded-xl border border-[var(--color-border)] px-4 font-semibold">إنهاء الجلسة</button>
          </div>
        </div>
        <nav className="mx-auto flex max-w-[1600px] gap-2 overflow-x-auto px-4 pb-3 md:px-8">
          {tabs.map(([to, label]) => <NavLink key={to} to={to} className={({ isActive }) => `min-h-11 shrink-0 rounded-xl px-5 py-3 text-sm font-bold ${isActive ? 'bg-[var(--color-text)] text-[var(--color-on-primary)]' : 'bg-[var(--color-bg)] text-[var(--color-text-muted)]'}`}>{label}</NavLink>)}
        </nav>
      </header>
