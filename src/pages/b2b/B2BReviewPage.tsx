...
export default function B2BReviewPage() {
  ...
  const [state, setState] = useState<'idle' | 'submitting' | 'error' | 'offline'>('idle')
  ...
  if (state === 'error' || state === 'offline') return <main className="grid min-h-screen place-items-center bg-[var(--color-bg)] px-4 text-center"><div className="max-w-sm"><div className="mx-auto grid size-20 place-items-center rounded-full border border-[var(--color-border)] bg-white text-3xl">!</div><h1 className="mt-6 text-2xl font-bold">{state === 'offline' ? 'لا يوجد اتصال بالإنترنت' : 'لم يتم تسجيل الطلب'}</h1><p className="mt-3 leading-7 text-[var(--color-text-muted)]">بياناتك محفوظة بالكامل ويمكنك إعادة المحاولة بأمان.</p><Button size="lg" className="mt-6 w-full" onClick={submit}>إعادة المحاولة</Button><Button size="lg" variant="secondary" className="mt-3 w-full" onClick={() => setState('idle')}>العودة للمراجعة</Button></div></main>
