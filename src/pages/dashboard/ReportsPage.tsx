...
export default function ReportsPage() {
  ...
  const report = buildRequestReport(items)
  ...
  return (
    <main className="space-y-6 p-4 lg:p-8" dir="rtl">
      <section className="flex flex-wrap items-end justify-between gap-4 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
        ...
        <button type="button" onClick={() => void load()} className="min-h-11 rounded-lg border border-[var(--color-border)] px-4 text-sm font-semibold outline-none hover:bg-[var(--color-bg)] focus-visible:ring-2 focus-visible:ring-[var(--color-accent)] focus-visible:ring-offset-2">تحديث</button>
      </section>
